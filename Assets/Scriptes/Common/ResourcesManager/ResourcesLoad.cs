...
public class ResourcesLoad
{
    ...
    public IEnumerator WWWLoad()
    {
        ...
        if (assetBundle != null)
        {
            ...
            else
            {
                ...
                {
                    if (isLoadSyn)
                        objectLoad = assetBundle.Load(assetName, typeof(Object));
                    else
                    {
                        Debug.Log("WWWLoad 33");
                        AssetBundleRequest abReq = assetBundle.LoadAsync(assetName, typeof(Object));
                        yield return abReq;
    ...
    public GameObject ObjectInstantiate()
    {
        ...
        if(reused)
            ObjectPool.CreatePool((GameObject)objectLoad, 10);
